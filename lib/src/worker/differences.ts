...
const getTranslates = (dimensions: ChildParentDimensions) => {
  ...
  const [originParentReferenceLeft, originParentReferenceTop] =
    parentReference.transformOrigin;
  ...
  const currentLeftDifference =
    current.currentLeft +
    originCurrentLeft -
    (parent.currentLeft + originParentCurrentLeft);
  ...
  const currentTopDifference =
    current.currentTop +
    originCurrentTop -
    (parent.currentTop + originParentCurrentTop);
...
const getScales = (dimensions: ChildParentDimensions) => {
  ...
  const parentWidthDifference =
    parent.currentWidth / parentReference.currentWidth;
  const parentHeightDifference =
    parent.currentHeight / parentReference.currentHeight;
  ...
  const textHeightCorrection =
    (current.currentHeight - reference.currentHeight) /
    2 /
