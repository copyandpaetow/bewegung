...
const getMaximumDimensions = (styleMap: calculatedElementProperties[]): MaximumDimensions => {
	const maxHeight = highestNumber(styleMap.map((prop) => prop.dimensions.height));
...
> => {
	const imageAnimationMap = new Map<string, { wrapper: PreAnimation; image: PreAnimation }>();
	...
	elementState.forEach((elementKey, idString) => {
		...
		const wrapperKeyframes = getWrapperKeyframes(styleMap, maxDimensions, easings);

		const imageKeyframes = calculateImageKeyframes(styleMap, maxDimensions, easings);
