...
import { getComputedStylings } from "../read-dom/read-element-properties";
import {
...
export const calculateDimensionDifferences = (
	...
	const [originReferenceLeft, originReferenceTop] =
	...
	const [originCurrentLeft, originCurrentTop] =
		parseTransformOrigin(currentEntry);
	...
	const currentXDifference =
		current.left +
		originCurrentLeft -
		(parentCurrent.left + originParentCurrentLeft);
	const referenceXDifference =
		reference.left +
		originReferenceLeft -
		(parentReference.left + originParentReferenceLeft);

	const currentYDifference =
		current.top +
		originCurrentTop -
		(parentCurrent.top + originParentCurrentTop);
	const referenceYDifference =
		reference.top +
		originReferenceTop -
		(parentReference.top + originParentReferenceTop);
